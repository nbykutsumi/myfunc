...
class IBTrACS(object):
  ...
  def ret_dlonlat(self,iDTime,eDTime, ver="v04r00",dhours=6,natures=['TS'],tracktypes=["PROVISIONAL","main"],lvaridx=[]):
    ...
    for line in lines:
      ...
      date     = list(map(int, isotime[0].split("-")))
      Year     = date[0]
      Mon      = date[1]
      Day      = date[2]
